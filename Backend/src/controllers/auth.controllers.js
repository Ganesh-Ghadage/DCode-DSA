...
export const resendVerificationMail = asyncHandler(async (req, res) => {
	...
	try {
		...
		return res
			...
			.json(
				new ApiResponce(
					200,
					updateduser,
					"Verification mail sent successfully, Kindly check your mail"
				)
			);
	} catch (error) {
		console.log(error);
		...
		throw new ApiError(
			error?.statusCode || 500,
			error?.message || "Verification mail sent failed"
		);
	}
});
...
export const refreshAccessToken = asyncHandler(async (req, res) => {
	...
	try {
		...
		throw new ApiError(
			error?.statusCode || 500,
			error?.message || "Something went worng, token are not generated",
			error
		);
	}
});
...
export const forgotPassword = asyncHandler(async (req, res) => {
	...
	try {
		...
		return res
			...
			.json(
				new ApiResponce(
					201,
					updatedUser,
					"Forgort password link sent successfully, check email for link"
				)
			);
	} catch (error) {
		console.log(error);
		...
		throw new ApiError(
			error?.statusCode || 500,
			error?.message || "Failed to generate forgot password link",
			error
		);
	}
});
